  ...
  results.sizes.forEach((size, i) => {
    ...
    tableHTML += `
      <tr>
        ...
        <td>${recTime === "overflow" ? '<span class="warning">Overflow</span>' : recTime.toFixed(5)}</td>
        <td>${ratio}</td>
      </tr>
    `;
  });
  ...
  manualChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'Perbandingan Waktu Eksekusi (Manual)',
          font: { size: 14, weight: 'bold' }
        },
        tooltip: {
          callbacks: {
            label: ctx => ctx.parsed.y.toFixed(6) + ' ms'
          }
        }
      },
      scales: {
